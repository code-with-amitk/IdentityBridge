:toc:
:toclevels: 3

= Scaling to 1.5 Million Req/sec
- Collector sends batches of 100-200 requests

== Collection Ingestion Architecture
Broken into 2 parts: Ingestion + Query

=== Ingestion Layer
Pod Scaling:
```
1 pod = 20K RPS
5 pods = 100K RPS
50 pods = 1000K RPS
100 pods = 20000k = 20M RPS
```
Ingestion
```mermaid
flowchart LR

    DNS["DNS<br>Route 53"]
    C1["Collecto1r<br/>Batched JSON"]
    C2["Collector2<br/>Batched JSON"]
    GLB["Global<br>LB"]
    RLB["Regional<br>LB"]


    subgraph Datacenter
        APIGW["API GW<br/>TLS termination"]
        LB["ELB/ALB"]
        subgraph JIMS Server
            subgraph API["Ingestion Tier"]
                KLB["k8s<br>Ingress LB"]
                P1["Pod 1<br/>Tokio Runtime<br> Kafka Producer"]
                P2["Pod 2<br/>Tokio Runtime<br> Kafka Producer"]
                PN["Pod 200<br/>Tokio Runtime<br> Kafka Producer"]
            end
            K[["Kafka<br>Broker<br><br> Partition0<br>Partition1<br>Partition2"]]
            subgraph Workers["Consumer Tier"]
                W1["Worker Pod 1<br><br>Partition0"]
                W2["Worker Pod 20<br><br>Partition1,2"]
            end
            DB["PostgreSQL"]
        end
    end

    C1 -->|hostname|DNS
    DNS -->|IP|C1

    C1 -->GLB
    C2 -->GLB
    GLB --> RLB
    RLB --> APIGW
    APIGW --> LB
    LB --> KLB
    KLB --> P1
    KLB --> P2
    KLB --> PN

    P1 -->|Topic:<br>identity-events| K
    P2 --> K
    PN --> K

    K -->|consumer group| W1
    K --> W2

    W1 --> DB
    W2 --> DB
```
